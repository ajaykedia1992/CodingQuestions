...
i and j exists.  Each node is an integer between 0 and graph.length - 1.  There are no self edges or parallel edges: graph[i]
...
public class IsGraphBipartite
{
    public boolean isBipartite(int[][] graph) {
        ...
        for(int i = 0; i < n ; i++){
            if(colors[i] == 0 && !checkBoundary(graph, i, colors, colorCode)){
                return false;
            }
        }
        return true;
    ...
    public boolean checkBoundary(int[][] graph, int vertex, int[] colors, int colorCode){

        if(colors[vertex] != 0){
            return colors[vertex] == colorCode;
        }

        colors[vertex] = colorCode;

        for(int v : graph[vertex]){
            if(!checkBoundary(graph, v, colors, -colorCode)){
                return false;
            }
        }
        return true;
